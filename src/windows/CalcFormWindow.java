...
/* TODO:
   Bugtest
   Gui streamline - add subheaders to pages. sub headers would be like "Cash and Cash Equivalent Assets" for page 1.
   clean up code & comment sections   
   Maybe add "stylize" function for labels to avoid repeated lines of code for the font and color change
   read enum names instead of string for pages
*/
...
{   
   ...
   // Look into getting enum names rather than hard coding
  ...
  public void updatePage() {
      ...
      for (int i = 0; i < pages.get(pageNum).length; i++) {
         ...
         TextField amount = new TextField((pageNum > 3) ? "-" : "");
