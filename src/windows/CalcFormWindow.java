...
/**
 * Displays a form to the user to input their assets and liabilities.
 * 
 * @author Davis
 */
public class CalcFormWindow extends Stage {
    ...
    // headerBox - holds header information like the header label and the pageCount
    // Label
    private HBox headerBox = new HBox();

    // Header datafields
    private Label header = new Label("Please complete the form below: ");
    private Label subHeader = new Label();
    ...
    // entryRow - arraylist to hold the HBoxes that contain a fiscalLabel and amount
    // textfield
    private ArrayList<HBox> entryRows = new ArrayList<HBox>();

    // pages - arraylist that contains the string arrays that contain the
    // fiscalLabels for each respective page
    private ArrayList<String[]> pages = new ArrayList<String[]>();

    /* String arrays for each page */
    // Cash and Cash Equivalent Assets
    private String page1[] = { "Checking Acounts", "Savings Acounts", "Money Market Acounts", "Savings bonds", "CD's",
            "Cash Value of Life Insurance" };

    // Invested Assets
    private String page2[] = { "Brokerage", "IRA", "Roth IRA", "401(k) or 403(b)", "SEP-IRA", "Keogh or equivalent",
            "Pension (vested benefit)", "Annuity" };

    // Business ownership Assets
    private String page3[] = { "Real estate", "Sole proprietorship", "Partnership", "C Corporation", "S Corporation",
            "Limited liability company", "Other" };

    // Use Assets
    private String page4[] = { "Principal Home", "Vacation Home", "Cars, Trucks, Boats", "Home Furnishings",
            "Art, Antiques, Coins, Collectibles", "Jewlery, Furs", "Other" };

    // Current Liabilities
    private String page5[] = { "Credit card balances", "Estimated income tax owed", "Other outstanding bills", "Home mortgage",
            "Home equity loan" };

    // Long-Term Liabilities
    private String page6[] = { "Mortgages on rental properties", "Car loans", "Student loans", "Life insurance policy loans",
            "Other long-term debt" };

    // Create next button
    private Button bttnNext = new Button("Next ->");
    private Button save = new Button("Save & Exit");

    // Make stage
    private Stage calcFormWindow = new Stage();

    // Create Record for this calcualation sequence
    private UserRecord record = UserRecord.createNewRecord("temp");
    ...
    /**
     * Function to move to the next page of net worth entries. Will update
     * UserRecord object with entries.
     *
     * @author Davis
     */
    ...
    /**
     * Function to save the input entries to the UserRecord object generated at the
    ...
    private void saveEntries() {
        ...
        for (int i = 0; i < pages.get(pageNum - 1).length; i++) {
            ...
            record.addEntry(UserRecord.FiscalEntryType.values()[enumCounter],
                    Double.parseDouble(((TextField) entryRows.get(i).getChildren().get(1)).getText()));
            System.out.println("Added value: " + record.getEntryByType(UserRecord.FiscalEntryType.values()[enumCounter])
                    + " to enum: " + UserRecord.FiscalEntryType.values()[enumCounter]);
            enumCounter++;
        }
    }
     ...
     * @author Davis
     */
    private double calcNetW(UserRecord record) {
        ...
        for (UserRecord.FiscalEntryType t : UserRecord.FiscalEntryType.values()) {
            System.out.println(
                    (netWorth + record.getEntryByType(t)) + " = " + record.getEntryByType(t) + " + " + netWorth);
    ...
    /**
     * Function to display the calculated networth to the user. The final page of
     * the form.
     *
     * @author Davis
     */
